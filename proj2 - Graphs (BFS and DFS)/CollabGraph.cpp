...
void CollabGraph::populate_graph(const vector<Artist>& artists) {
    ...
    // Create vertices for each artist
    ...
    for (size_t i = 0; i < artists.size(); i++) {
        ...
        for (size_t j = 0; j < artists.size(); j++) {
            ...
            if (i != j) {
                ...
                // Get the song they collabed on
                string song = artists.at(i).get_collaboration(artists.at(j));

                // If the song exists, create the edge
                if (song != "") {
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist& artist) const {
    ...
    for (size_t i = 0; i < v->neighbors.size(); i++) {
        ...
        neighbors.push_back(v->neighbors.at(i).neighbor->artist);
...
stack<Artist> CollabGraph::report_path(const Artist& source,
    ...
    // if source or dest are not in the graph
    if ((not is_vertex(source)) or (not is_vertex(dest))) {
    
        return path;
    }

    // if their is no valid path btw source and dest
    if (graph.at(dest.get_name())->predecessor == nullptr) {
        return path;
    }
    
    // the destination should be the bottom-most element of the stack
    path.push(dest);
    Artist curr = dest;

    // Loop back through the predecessors and add them to the stack
    // Stop when the source is reached
    while (get_predecessor(curr) != source) {
